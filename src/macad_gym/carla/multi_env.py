...
class MultiCarlaEnv(*MultiAgentEnvBases):
        ...
        if self._render and self._env_config.get("spectator_loc"):
            ...
            spectator.set_transform(
                carla.Transform(location,
                                carla.Rotation(yaw=30 + angle, pitch=-60)))
    ...
    def _step(self, actor_id, action):
        ...
        if self._squash_action_logits:
            ...
            forward = 2 * float(softmax(action[0]) - 0.5)
            throttle = float(np.clip(forward, 0, 1))
            brake = float(np.abs(np.clip(forward, -1, 0)))
            steer = 2 * float(softmax(action[1]) - 0.5)
        else:
            throttle = float(np.clip(action[0], 0, 0.8))*10 #*5 extra
            brake = float(np.abs(np.clip(action[0], -1, 0)))
            steer = float(np.clip(action[1], -3, 3))
        reverse = False
        hand_brake = False
        if self._verbose:
        ...
        if config["log_measurements"] and CARLA_OUT_PATH:
            ...
            if not self._measurements_file_dict[actor_id]:
                self._measurements_file_dict[actor_id] = open(
                    os.path.join(
                        CARLA_OUT_PATH,
                        "measurements_"+actor_id+"_{}.json".format(
                            self._episode_id_dict[actor_id]),
                    ),
                    "w",
                )
            dumpy = json.dumps((py_measurements), cls=NumpyArrayEncoder)     
...
def diffdistance(self, point1, point2):
        """
