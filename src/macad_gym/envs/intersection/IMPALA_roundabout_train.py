...
class IMPALA_roundabout_train(MultiCarlaEnv):
    """A 4-way signalized intersection Multi-Agent Carla-Gym environment"""
    def __init__(self):
        self.configs = {
            "scenarios": "Roundabout_IMPALA",
            "env": {
        ...
        super(IMPALA_roundabout_train, self).__init__(self.configs)


if __name__ == "__main__":
    env = IMPALA_roundabout_train()
    configs = env.configs
    for ep in range(2):
        obs = env.reset()
