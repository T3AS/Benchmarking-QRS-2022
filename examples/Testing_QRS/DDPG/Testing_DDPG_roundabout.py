...
# Used only in debug mode
env_name_3 = "DDPG_roundabout_train-v0"
...
agents_reward_dict = {'carDDPG': 0.0}
done = False
i=0
action = {}
with open("info_carDDPG.json", "w") as f1:
    ...
    while i < 5000:  # number of steps in a episodic run 
        ...
        for agent_id in reward:
            ...
            if agent_id=="carDDPG":
                # n_lis = [(key, value) for key, value in info[agent_id].items()] 
                # pprint (type(n_lis))
                # m = list(zip(info[agent_id].keys(), info[agent_id].values()))
                # lists = info[agent_id].toarray()
                # json.dump(info[agent_id], codecs.open("info_car2DDPG.json", 'w', encoding='utf-8'), separators=(',', ':'), sort_keys=True, indent=4)
               
               
               
                # listt = [(k, v) for k, v in info[agent_id].items()]

                json_dump = json.dumps(info[agent_id], cls=NumpyEncoder)
                # json.dump(json_dump, f1)
                # pprint (json_dump)
                f1.write(json_dump)
                f1.write("\n")
