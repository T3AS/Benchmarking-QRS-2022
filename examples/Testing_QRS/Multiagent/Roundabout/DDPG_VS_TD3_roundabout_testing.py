...
parser.add_argument(
    ...
    default='/home/aizaz/Desktop/PhD-20210325T090933Z-001/PhD/10_August_2022/Benchmarking-Archive/examples/Training_QRS/DDPG/Roundabout/DDPG_Roundabout/DDPG_DDPG_roundabout_train-v0_0_2022-08-19_16-27-20ak2yr263/checkpoint_50/checkpoint-50',    
    help="Path to checkpoint to resume training")
parser.add_argument(
    "--checkpoint-path2",
    #Replace it with your path of last training checkpoints
    default='/home/aizaz/Desktop/PhD-20210325T090933Z-001/PhD/10_August_2022/Benchmarking-Archive/examples/Training_QRS/TD3/Roundabout/TD3_Roundabout/DDPG_TD3_roundabout_train-v0_0_2022-08-20_12-03-4288z2h369/checkpoint_50/checkpoint-50',
    help="Path to checkpoint to resume training")
parser.add_argument(
    "--checkpoint-path7",
    #Replace it with your path of last training checkpoints
    default='/home/aizaz/ray_results/DDPG-TD3-Roundabout/MultiAgent_DDPG_TD3_roundabout_train-v0_0_2022-08-30_11-21-493fmmt5x1/checkpoint_1/checkpoint-1',
    help="Path to checkpoint to resume training")
parser.add_argument(
    "--disable-comet",
    action="store_true",
...
env_name = "DDPG_roundabout_train-v0"
...
# Used only in debug mode
env_name_2 = "TD3_roundabout_train-v0"
...
# Used only in debug mode
env_name_7 = "DDPG_TD3_roundabout_train-v0"
...
def env_creator(env_config):
    ...
    import macad_gym
    env = gym.make("DDPG_roundabout_train-v0")
...
def env_creator_2(env_config):
    ...
    # curriculum learning or joint training experiments
    import macad_gym
    env = gym.make("TD3_roundabout_train-v0")
...
def env_creator_7(env_config):
    ...
    # curriculum learning or joint training experiments
    import macad_gym
    env = gym.make("DDPG_TD3_roundabout_train-v0")
...
def default_policy_2():
   ...
   # "gamma": 0.99,
    ...
    # pprint (config)
    return (DDPGTFPolicy, Box(0.0, 255.0, shape=(84, 84, 3)), Box(low=-1, high=1, shape=(2,)),config_2)
...
policyDDPG = new_trainer.get_policy("carDDPG")
weights_as_np_DDPG = policyDDPG.get_weights()



new_trainer_2 = ddpg.DDPGTrainer(
    env=env_name_2,
    # Use independent policy graphs for each agent
    config={
...
policyTD3 = new_trainer_2.get_policy("carTD3")
weights_as_np_TD3 = policyTD3.get_weights()
...
policies = {
        "carDDPG": default_policy(),
        "carTD3": default_policy_2(),
        
    }
MyTrainer = build_trainer(
        name="DDPG_TD3_Roundabout",
        default_policy=None)
...
trainer = MyTrainer(
    ...
    config={
        ...
        "multiagent": {
            "policies": {"carDDPG": default_policy(),
        "carTD3": default_policy_2(),

            },
            "policy_mapping_fn": lambda agent_id: agent_id,
        },
        "env_config": env_actor_configs_7,
        "num_workers": args.num_workers,
        "num_envs_per_worker": args.envs_per_worker,
        "sample_batch_size": args.sample_bs_per_worker,
        # "rollout_fragment_length": args.sample_bs_per_worker,
...
trainer.get_policy("carDDPG").set_weights(weights_as_np_DDPG)
trainer.get_policy("carTD3").set_weights(weights_as_np_TD3)
...
agents_reward_dict = {'carDDPG': 0.0, 'carTD3': 0.0}
done = False
i=0
action = {}
with open("info_carDDPG.json", "w") as f1, open("info_carTD3.json", "w")  as f2:
    ...
    while i < 5000:  # number of steps in a episodic run 
        ...
        step_reward=0
        step_reward += reward["carDDPG"]+reward["carTD3"]
        ...
        episode_reward += reward["carDDPG"]+reward["carTD3"]
